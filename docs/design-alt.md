...
3. **Behavior conformance — golden fixtures over seeded data.** Schema
   ...
   replays fixed request → expected-response pairs, generated against
   seeded known data, byte-for-byte against every implementation over
   HTTP. `spec/fixtures/parser-tests.json` and
   `filter-builder-tests.json` are the first instance of this pattern,
   scoped to the filter DSL; the conformance kit generalizes it to every
   route.
