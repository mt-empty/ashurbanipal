...
| SQLite   | Live `GROUP BY ... ORDER BY COUNT(*) DESC LIMIT 20` | No `pg_stats` analog exists. The spike computes frequencies from a bounded, capped live aggregate (`COMMON_VALUES_LIMIT = 20` in `sqlite.rs`) rather than reading pre-computed stats. This is a real relaxation of "never a data query" — acceptable for a spike because the query is `GROUP BY` + `LIMIT`, not an unbounded `SELECT DISTINCT`, but it does real I/O proportional to table size on every call, unlike the Postgres path. Worth reconsidering (e.g. caching, or reading SQLite's own `ANALYZE`-produced `sqlite_stat1`/`sqlite_stat4` tables when present) before promotion. |
...
| SQLite   | `CAST(column AS TEXT)` in the select list | Same property, different syntax — SQLite has no `::` cast operator. SQLite's declared column types can be the empty string (dynamic typing); the spike falls back to the literal `"unknown"` type-name label rather than emitting `""` (`sqlite.rs::query_table`). |
...
The SQLite adapter (`implementations/rust/src/db/sqlite.rs`) is currently
a spike gated behind the `sqlite` Cargo feature, not a listed/conformant
port — see `docs/design.md` §2's non-Postgres-backends non-goal and
`PORTING.md`'s listing bar. The rows above describe the decisions the spike
already made; promoting it to a real port means running those decisions
back through the protocol governance process referenced above, not just
