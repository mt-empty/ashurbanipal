...
| Backend  | Mechanism | Notes |
|----------|-----------|-------|
| Postgres | `list_tables`/`table_counts` and the `table` allow-list (`query_table`/`common_values`) all use the identical `pg_class`/`pg_namespace`, `relkind = 'r'` predicate, each gated by `has_table_privilege(…, 'SELECT')` | `has_table_privilege` keeps the listing, the counts, and the allow-list on "the role can actually read it" — without it, an `INSERT`-only table would clear the allow-list and then fail the row fetch with a raw `permission denied` 500; a residual 42501 there is still mapped to `NotAllowed` (→ 400) at the fetch site. The allow-list used to be sourced from `information_schema.tables`' `BASE TABLE` instead — a broader set that (unlike `relkind = 'r'`) also matches partitioned tables, so `query_table`/`common_values` silently accepted a `table` that `list_tables` itself excludes. Fixed to share `list_tables`' exact predicate (`referenced_by`, §5.9, already used a dedicated `relkind = 'r'` check of its own for the same reason and keeps it separate rather than being re-collapsed into this one). Implemented in all five ports' Postgres sources; each has a port-local integration test (`table_listing_privileges` / `TableListingPrivilegesTest`). |
...
## §5.4.1 — forward FK/PK column metadata (`key_metadata`)

Protocol property: for a given `table`, each column's `key`
(pk/fk/omitted) and, for an FK column, its `references` target — the
per-column forward direction (§5.9 is the reverse, catalog-only scan).
Composite FKs are omitted rather than partially reported, since §5.4.1
has no way to express a multi-column `references` on one column.

| Backend  | Mechanism | Notes |
|----------|-----------|-------|
| SQLite   | `pragma_foreign_key_list(table)`, left-joined to `pragma_table_info(fkl."table")` for a parent primary-key column lookup, keyed per FK row rather than once — see the §5.9 SQLite row for the shared mechanism | Same underlying bug as §5.9's SQLite row: `pragma_foreign_key_list.to` is NULL for `REFERENCES parent` DDL with no parenthesised parent-column list (a very common shorthand), and the fix is the same `pragma_table_info`/`COALESCE(..., 'rowid')` resolution. The one structural difference: §5.9 walks FKs *into* one fixed parent, so its `pragma_table_info` lookup is a single bound parameter; here, one table's own outgoing FKs can each reference a *different* parent, so the lookup is a lateral join keyed off each FK row's own `"table"` column instead. |
...
| Backend  | Mechanism | Notes |
|----------|-----------|-------|
| Postgres | `pg_catalog.pg_constraint` (`contype = 'f'`, `conparentid = 0`, `confrelid` = the target table's OID), joined to `pg_class`/`pg_namespace` for the referencing table's schema and `unnest(conkey, confkey) WITH ORDINALITY` ⨝ `pg_attribute` for the column pairs; gated in-query by `rc.relkind = 'r' AND has_table_privilege(con.conrelid, 'SELECT')`; `table` itself is gated by a dedicated `relkind = 'r' AND has_table_privilege(…, 'SELECT')` existence check — one targeted row, not a full per-schema table listing | The `information_schema` reverse query (the forward `table_constraints`/`key_column_usage`/`constraint_column_usage` join with the filter moved to the referenced side) is correct but measured 18–140 ms on a ~2,000-table catalog — the standard-SQL views have no predicate push-down, expand `conkey` per constraint, and call `has_*_privilege` per row; the same tax the forward `key_metadata` query already pays. `pg_catalog` filtered on `confrelid` is ~1–6 ms at that size (see `docs/feature-backlog/14-incoming-fk-references.md` for the measurements). The `confrelid` predicate is a small sequential scan of `pg_constraint` — there is no `confrelid` index — but the catalog is tiny; the target OID is resolved with a bound-parameter `pg_class`/`pg_namespace` lookup, never a `::regclass` cast on request text. The in-query `has_table_privilege` gate is the whole privilege story: it works per-referrer regardless of schema, so cross-schema referrers are included (and gated), and there is no separate full-per-schema-list intersection (which would be single-schema and would drop them). This is the one place the Rust Postgres source reads FK topology from `pg_catalog` rather than `information_schema` (§6 server invariant "no unvalidated identifier reaches SQL" is unaffected — the request's `table` is already allow-list-validated, and every referencing name in the result is allow-list-checked before any §5.4 drill-in — `rc.relkind = 'r'` is part of that check, see the third correction below). Three partitioning-specific corrections: Postgres clones a partitioned table's FK constraint onto every partition, so a naive `confrelid` scan reports one entry per partition plus the parent for what is one logical relationship — `conparentid = 0` keeps only the parent's own (non-inherited) copy. And `allowed_tables`/`information_schema.tables` reports `table_type = 'BASE TABLE'` for a partitioned table even though it's excluded from §5.2's own `list_tables` (`relkind = 'r'`); gating `table` on that broader definition let a partitioned target clear the check and then match zero rows in the `relkind = 'r'` `confrelid` subselect, silently returning `[]` instead of the 400 an unlisted table gets. The dedicated existence check uses `list_tables`' exact predicate so the two can't diverge again. Third: `conparentid = 0` alone still lets a partitioned table's own (non-inherited) FK constraint through when *it* is the referrer — its own name then names a table `list_tables`/`query_table`/`common_values` all reject, an entry the frontend can't drill into. `rc.relkind = 'r'` closes that: `pg_class`'s only relkinds that can carry a real FK constraint as `conrelid` are `r` (ordinary table) and `p` (partitioned table) — views, materialized views, and foreign tables cannot — so this predicate excludes exactly the partitioned-parent case and nothing else. |
| MySQL/MariaDB | `information_schema.key_column_usage` predicate-flip: `WHERE referenced_table_schema = ? AND referenced_table_name = ? AND table_schema = ?` (the last clause scopes to the resolved database — cross-database FKs are not surfaced). Match on `key_column_usage`, **not** `referential_constraints`: MariaDB nulls `referential_constraints.referenced_table_name` for a role lacking privilege on the *referenced* table, while `key_column_usage.referenced_table_name` stays populated (verified on MariaDB 11.8). Then intersect the referencing table names against the un-gated `information_schema.tables` allow-list | MySQL 8.0+'s `information_schema` is an indexed view over the transactional data dictionary, so this stays fast — no `pg_catalog` escape hatch needed (MariaDB keeps the pre-8.0 `.frm`-era catalog and is slower on a large schema, but on-demand once per panel open it is acceptable, and `information_schema.INNODB_SYS_FOREIGN` — its only faster alternative — needs the server-wide `PROCESS` privilege). `schema` is never emitted (the result is single-database by construction). Neither engine has a `has_table_privilege` analog (no scalar privilege predicate at all — verified against MySQL 8.4 / 9.x and MariaDB 11.8 / 12.x docs), so the privilege gate is only partial: `information_schema` rows are already privilege-scoped, so a referencing table the role has **no** privilege on is not listed at all (verified on both engines); one the role has *some* privilege on (INSERT/UPDATE/REFERENCES/…) but not `SELECT` still lists, and fails the later §5.4 row fetch with `ER_TABLEACCESS_DENIED_ERROR` (1142) → `NotAllowed` (400) — the same residual-mapping gap already documented for the forward direction in `## §5.2 / §5.3`. `key_column_usage` itself needs no special privilege (unlike `INNODB_SYS_FOREIGN`). MariaDB 12.1+ makes FK constraint names unique per table rather than per database, so `constraint` alone is not a response-wide key there; `(table, constraint)` still is (`spec/protocol.md` §5.9). Unlike the Postgres row above, this mechanism needs no partitioning-specific correction: InnoDB does not support foreign keys on a partitioned table in either direction (as referrer or as target), so the partition-inherited-constraint duplication has no MySQL/MariaDB analog — not an unaudited gap. (The SQLite row's separate NULL-`to` correction doesn't apply here either, for an unrelated reason: MySQL's `FOREIGN KEY (…) REFERENCES tbl (…)` always requires an explicit parent column list — there is no shorthand form to omit it.) |
| SQLite   | `sqlite_master` ⨝ `pragma_foreign_key_list(m.name)` table-valued function (SQLite 3.16+), left-joined to `pragma_table_info(table)` for a parent primary-key column lookup; `WHERE fkl."table" = ? COLLATE NOCASE` | No reverse-FK index and no `information_schema` — but the whole schema is parsed into memory when the DB opens, so scanning every table's `pragma_foreign_key_list` is single-digit ms even for thousands of tables. The TVF argument is a column reference (no splicing); `fkl."table" = ?` is an ordinary bound predicate. `pragma_foreign_key_list` returns the referenced table name as written in the `REFERENCES` clause (DDL text), while SQLite resolves names case-insensitively — `COLLATE NOCASE` closes that gap. FK constraints are unnamed in SQLite, so `constraint` is synthesized as `fk_<id>` from `pragma_foreign_key_list.id` (stable within one catalog read, not across schema changes — spec/protocol.md §5.9 permits this). `schema` is never emitted (single schema). No privilege model, and the `sqlite_master` predicate here (`type = 'table'`, no `sqlite_%`) is byte-identical to `allowed_tables()`, so every referrer it yields is already allow-listed — no separate intersection step, unlike Postgres/MySQL. The request `table` is still allow-list-checked before the query runs. `pragma_foreign_key_list.to` is NULL for `REFERENCES parent` DDL with no parenthesised parent-column list — valid SQL meaning "the parent's own primary key" — a very common shorthand, not an edge case. `pragma_table_info(table)`'s `pk` column (1-indexed position within the primary key) resolves it against `fkl.seq` (0-indexed position within the FK's own column list); SQLite requires an omitted column list to align position-for-position with the parent PK, so this pairing holds for composite keys too. A `COALESCE(..., 'rowid')` fallback covers the rarer case of a parent with no declared primary key, where SQLite's parent key is the implicit rowid. |
